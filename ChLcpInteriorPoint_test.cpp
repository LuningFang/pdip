...
{
	std::cout << "-------time for lulu's solver!!------" << std::endl;
...
//		if (stage == 39)
//			print = true;
		bool solveSuc = solveSPIKE(nBodies, numContacts, Cq_i, Cq_j, Cq_nnz, Cq_val, Minv_i, Minv_j, Minv_val, BlockDiagonal_i, BlockDiagonal_j, BlockDiagonal_val, spike_dx, spike_rhs, print);
...
*/
				...
				ChMatrixDynamic<double> grad_f_padded(3*nc_original,1);
				sysd.ShurComplementProduct(grad_f_padded, &xk_tmp_padded, 0);
...
*/		}
		...
		if ((r_cent.NormInf() < res1 && r_dual.NormInf() < res2) || stage+1 >= numStages){
						//			fprintf(stderr, "stage[%d], rd = %e, rg = %e, s = %f, t = %f\n", stage+1, r_dual.NormInf(), r_cent.NormInf(), ss, barrier_t);
...
std::vector<int> ChLcpInteriorPoint::findModifiedIndex(int nc, double res){
