	...
	virtual void Increment_q(const double deltal)
					{
						if (variables_a->IsActive()){
						 for (int i= 0; i < 6; i++){
//							fprintf(stderr, "Body A: q[%d] = %g, ", i, variables_a->Get_qb()(i));
//							fprintf(stderr, "M^[-1]D: %g * dq: %g ", Eq_a.ElementN(i), deltal);
							variables_a->Get_qb()(i) += Eq_a.ElementN(i) * deltal;
//							fprintf(stderr, "updated: %g\n", variables_a->Get_qb()(i));
					 }
						}
						if (variables_b->IsActive()){
						 for (int i= 0; i < 6; i++){
//							fprintf(stderr, "Body B: q[%d] = %g, ", i, variables_b->Get_qb()(i));
//							fprintf(stderr, "M^[-1]D: %g * dq: %g ", Eq_b.ElementN(i), deltal);
							variables_b->Get_qb()(i) += Eq_b.ElementN(i) * deltal;
//							fprintf(stderr, "updated: %g\n", variables_b->Get_qb()(i));
						 }
						}
					};
